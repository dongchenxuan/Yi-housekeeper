...
namespace GUI.UI
{
    ...
    public partial class 木马查杀 : Form
    {
        ...
        */
        ...
        #region 写入每次的扫描记录
        private void Writedaylog(string state)
        {
            string Handle_time = DateTime.Now.ToString("yyyy年MM月dd日HH:mm");
            string str;
            if (Scan_option == 1)
            {
                str = "快速扫描" + @"," + state + @"," + Handle_time;
            }
            else if (Scan_option == 2)
            {
                str = "全盘扫描" + @"," + state + @"," + Handle_time;
            }
            else
            {
                str = "自定义扫描" + @"," + state + @"," + Handle_time;
            }
            FileInfo finfo = new FileInfo(@".\\bin\\history.dat");
            if (finfo.Exists == false)//如果文件不存在
            {
                File.Create(@".\\bin\\history.dat");
            }
            //为文件打开一个二进制写入器
            FileStream fs = new FileStream(@".\\bin\\history.dat", FileMode.Append);
            StreamWriter wr = new StreamWriter(fs);
        ...
        private void 木马查杀_Paint(object sender, PaintEventArgs e)
        {
            Application.DoEvents();
